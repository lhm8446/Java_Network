...
public class RequestHandler extends Thread {
	...
	public void run() {
		try {
			...
			// get IOStream
			OutputStream outputStream = socket.getOutputStream();													 // 쓰기
			BufferedReader br = new BufferedReader( new InputStreamReader( socket.getInputStream(), "UTF-8" ) ); 	 // 읽기
			
			String request = null;               // request 가 null 일때만 , 즉 첫줄만 출력하기 위하여
			while( true ) {
	...
	public void responseStaticResource(OutputStream outputStream, String url, String protocol )throws IOException {
		...
		System.out.println( url + ":" + protocol );   //url = / , protocol = HTTP/1.1
